...
#define MAX_SLEEP 10000 // pretty small --mtoups
...
pid_t __wrap_fork(void)
{
    ...
    if (pid == 0) {
	if(bool) {
	    usleep(secs); // can be small for the child
	}
    }
    else {
	if (!bool) {
	    // printf("sleeping %d usec in parent.\n",40000+secs); // for debugging --mtoups
	    usleep(40000+secs); /* make this more pronounced in parent, since that triggers race cond */
	}
    }
