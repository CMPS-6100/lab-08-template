 ...
 * background children don't receive SIGINT or SIGTSTP from the kernel
 * when we type ctrl-c or ctrl-z at the keyboard.  
*/
void eval(char *cmdline) 
{
    return;
...
void do_redirect(char **argv)
{
        ...
        {
                if (!strcmp(argv[i],"<")) {          
                        FILE* f = freopen(argv[i+1], "r", stdin);
                        if(f == NULL) {
                        ...
                        FILE* f = freopen(argv[i+1], "w", stdout);
                        if(f == NULL) {
...
void do_bgfg(char **argv) 
{
    if(argv[1] == NULL){
        ...
        return; // do nothing if <job> not given
    }
    char* job_str = argv[1];
    int jid;
    int pid;
    if(job_str[0] == '%'){ // if jid given
        job_str++;
        jid = atoi(job_str);
        if(jid == 0) {
    ...
    if(strcmp(argv[0], "fg") == 0){
        ...
        // sent SIGCONT to continue the job
        kill(-pid, SIGCONT);
        waitfg(pid);
 ...
 *    user types ctrl-c at the keyboard. This function catches it.
 ...
 *     the user types ctrl-z at the keyboard. This function catches it
 *     and suspends the foreground job by sending it a SIGTSTP.  
 */
void sigtstp_handler(int sig) 
{
...
void sigchld_handler(int sig) 
{
    // get pid of child that stopped
    int status;
    int childPID = waitpid(-1, &status, WNOHANG | WUNTRACED | WCONTINUED);
    ...
    if(WIFSIGNALED(status)){
        int childSIG = WTERMSIG(status);
        printf("Job [%d] (%d) terminated by signal %d\n", jid, childPID, childSIG);
        deletejob(jobs, childPID);
    }
    if(WIFSTOPPED(status)){
        int childSIG = WSTOPSIG(status);
